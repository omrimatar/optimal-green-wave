  ...
  const renderDiagonalLines = () => {
    ...
    return pairBandPoints.map((pair, index) => {
      ...
      if (pair.down.origin_low < pair.down.dest_low) {
        // Calculate intersection with cycle time boundary
        const ratio = (pair.down.origin_low) / (pair.down.origin_low - (pair.down.dest_low - maxCycleTime));
      ...
      if (pair.down.origin_high < pair.down.dest_high) {
        // Calculate intersection with cycle time boundary
        const ratio = (pair.down.origin_high) / (pair.down.origin_high - (pair.down.dest_high - maxCycleTime));
        const intersectX = destX + ratio * (originX - destX);
