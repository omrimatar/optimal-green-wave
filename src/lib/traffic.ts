...
* שרשור UP
******************************************************************/
function chainBWUp(offsets: number[], data: NetworkData, travelUp: number[]): {chain_up_start: number|null, chain_up_end:number|null} {
    const n = data.intersections.length;
    if(n < 2) return {chain_up_start: null, chain_up_end: null};
    ...
    let Lc: number, Uc: number;
    {
        ...
        Lc = Math.max(a, b);
        Uc = Math.min(c, d);
        if(Lc > Uc) return {chain_up_start: null, chain_up_end: null};
    }
    ...
    for(let i = 1; i < n-1; i++) {
        ...
        if(newL > newU) return {chain_up_start: null, chain_up_end: null};
        Lc = newL;
        Uc = newU;
    }
...
* שרשור DOWN
******************************************************************/
function chainBWDown(offsets: number[], data: NetworkData, travelDown: number[]): {chain_down_start: number|null, chain_down_end: number|null} {
    const n = data.intersections.length;
    if(n < 2) return {chain_down_start: null, chain_down_end: null};
    ...
    let Lc: number, Uc: number;
    {
        ...
        Lc = Math.max(a, b);
        Uc = Math.min(c, d);
        if(Lc > Uc) return {chain_down_start: null, chain_down_end: null};
    }
    ...
    for(let i = n-2; i > 0; i--) {
        ...
        const newL = Math.max(Lc, Math.max(a, b));
        const newU = Math.min(Uc, Math.min(c, d));
        if(newL > newU) return {chain_down_start: null, chain_down_end: null};
        Lc = newL;
        Uc = newU;
    }
